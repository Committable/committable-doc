...
In this document, we will go over Meta-Commit, looking at both its structure definition (see *Section II: Specification*) and living example (see *Section III: Example*). To our newcomers, a *Meta-Commit* is an extension to the standard git commit object (just as those you see on Github or other similar system), which combines both common commit information, e.g., author, commiter, message, as well as more in-depth program analysis results. Sitting at the core of COMMITs, it is how we recognize and exploit what most developers know as git commit.

### II. Specification

A Meta-Commit contains complete pieces of information with regard to a specific git commit object, e.g., the commit with object id (also known as commit hash) `be3cfa0f9068a1495dc524975209f223da20e148` within the famous OpenZeppelin smart contract repository hosted at `https://github.com/OpenZeppelin/openzeppelin-contracts.git`. This kind of information is generated by process called "Minting", and is baked into the COMMIT ERC-721 token.

#### Standard Fields

Basically, Meta-Commit fields can be grouped into three categories:
